...
[InstallDelete]
Type: files; Name: "{app}\\cliphist.exe"
...
var
  ResultCode: Integer;
begin
  { Tray apps intercept WM_CLOSE (hide-to-tray), so Restart Manager is not enough. }
  Exec(ExpandConstant('{sys}\taskkill.exe'), '/F /IM my-cliphist.exe /T', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
  Exec(ExpandConstant('{sys}\taskkill.exe'), '/F /IM cliphist.exe /T', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
end;
